...
def _make_moving_person_obs(frame_idx: int, timestamp: float, cx: float, cy: float, h: float = 230, w: float = 90, tid: int = 1):
    x1, y1 = cx - w / 2, cy - h / 2
    x2, y2 = cx + w / 2, cy + h / 2
    return {
        "frame_idx": frame_idx,
        ...
        "tracks": [
            {"track_id": tid, "class": "insan", "history_length": 1, "last_center": [cx, cy], "speed": [0, 0]},
        ],
        "scene_graph": {"frame_idx": frame_idx, "timestamp": timestamp, "nodes": [], "edges": []},
    }


def test_immobility_triggers_for_stationary_person_with_jitter():
    """Gerçekten hareketsiz bir kişi, bbox tespit gürültüsü (jitter, +-2px rastgele)
    olsa da min_duration_seconds sonunda immobile_person üretmeli.

    Eşikler (window_seconds=3.0, movement_ratio_threshold=0.12) gerçek video
    ölçümünden kalibre edildi: Completely_stationary_overhead.mp4'te gerçekten
    hareketsiz bir kişide bu oran zamanla büyümüyor (~0.09'da tavan yapıyor,
    rastgele/sınırlı gürültü); bkz. config.yaml immobility bloğu yorumu.
    """
    import random
    random.seed(42)

    cfg = EventsConfig(
        enabled_rules=["immobility"],
        thresholds={"immobility": {"window_seconds": 3.0, "movement_ratio_threshold": 0.12, "min_duration_seconds": 4.0}},
    )
    engine = EventEngine(cfg, fps=24.0)

    base_cx, base_cy = 100.0, 300.0
    triggered = False
    for i in range(int(6 * 24.0)):
        jx = random.uniform(-2, 2)
        jy = random.uniform(-2, 2)
        signals = engine.process_observation(_make_moving_person_obs(i, i / 24.0, base_cx + jx, base_cy + jy))
        if any(s.event_type == "immobile_person" for s in signals):
            triggered = True
            break

    assert triggered, "Gerçekten hareketsiz kişi (jitter'lı) için immobile_person üretilmedi"


def test_immobility_not_triggered_for_slowly_walking_person():
    """Yavaş yürüyen bir kişi (gerçek video ölçümünden: ~32px/s), bbox jitter'ıyla
    karıştırılıp durağan sayılmamalı. Bu, eski (frame-to-frame, pencere yok)
    tasarımın kusuruydu: kare-kareye hareket küçük olduğu için hiç sıfırlanmıyor,
    yürüyen kişi 'durağan' tespit ediliyordu."""
    import random
    random.seed(42)
    ...
    cx, cy = 90.0, 540.0
    speed_px_per_sec = 32.0  # proximity.mp4 tid=1'den ölçülen gerçek hız
    vx_per_frame = speed_px_per_sec / 24.0
    triggered = False
    for i in range(int(6 * 24.0)):
        jx = random.uniform(-2, 2)
        jy = random.uniform(-2, 2)
        cx += vx_per_frame
        signals = engine.process_observation(_make_moving_person_obs(i, i / 24.0, cx + jx, cy + jy))
        if any(s.event_type == "immobile_person" for s in signals):
            triggered = True
            break
