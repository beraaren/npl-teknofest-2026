...
def build_system_prompt(rows: int, cols: int) -> str:
    ...
    return (
        ...
        f"kareleri içeren tek bir ızgara (grid) görseli veriliyor. Grid {rows}×{cols} boyutunda, "
        f"soldan sağa, yukarıdan aşağıya 0'dan başlayan pozisyon numaralarına sahiptir "
        f"(0-{max_idx} arası {total_cells} hücre). "
        "Görevin, olay motorundan ve nesne dedektöründen BAĞIMSIZ olarak, gördüğün "
        "sahneyi kendi gözünle yorumlaman. "
        ...
        '  "risk_events": [\n'
        '    {\n'
        '      "description_tr": "risk açıklaması Türkçe",\n'
        '      "severity": "low|medium|high|critical",\n'
        '      "confidence": 0.85,\n'
        f'      "supporting_frame_count": 2,\n'
        f'      "supporting_frame_positions": [1, 4]\n'
        '    }\n'
        '  ],\n'
        '  "confidence_overall": 0.0\n'
        "}\n"
        "risk_events KURALLARI: risk yoksa boş liste [] döndür. "
        "severity: low=düşük öncelik, medium=dikkat, high=önemli risk, critical=insan sağlığı tehlikesi. "
        f"supporting_frame_count: bu riski kaç grid karesinde gördün (0-{total_cells} arası). "
        f"supporting_frame_positions: riskin görüldüğü grid indeksleri (0=sol üst, soldan sağa, max={max_idx}). "
        "confidence: sadece bu riske özgü güven değerin (0.0-1.0)."
        # NEDEN YENİ ŞEMA?
...
def _extract_reasoning_and_json(text: str) -> tuple[str, dict]:
    ...
    #   <düşünme>   → build_system_prompt() içindeki Türkçe prompt şablonunun etiketi
